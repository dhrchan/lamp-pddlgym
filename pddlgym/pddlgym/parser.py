...
from pddlgym.structs import (Type, Predicate, Literal, LiteralConjunction,
                             ...
                             DerivedPredicate, NoChange, ConditionalEffect)
...
class PDDLParser:
    ...
    def _parse_into_literal(self, string, params, is_effect=False):
        ...
        if string.startswith("(when") and string[5] in (" ", "\n", "("):
            assert is_effect, "We only support conditional effects"
            expr = string[5:-1].strip()
            antecedent = self._find_balanced_expression(expr, 0)
            expr = expr[len(antecedent):].strip()
            consequence = self._find_balanced_expression(expr, 0)
            condition = self._parse_into_literal(antecedent, params, is_effect=False)
            effect = self._parse_into_literal(consequence, params, is_effect=True)
