...
from pddlgym.structs import ground_literal, Literal, State, ProbabilisticEffect, LiteralConjunction, NoChange, ConditionalEffect
...
def _compute_new_state_from_lifted_effects(state, lifted_effects, assignments, new_literals):        
    for lifted_effect in lifted_effects:
        if isinstance(lifted_effect, LiteralConjunction):
            _compute_new_state_from_lifted_effects(state, lifted_effect.literals, assignments, new_literals)
        elif isinstance(lifted_effect, ConditionalEffect):
            condition = ground_literal(lifted_effect.condition, assignments)
            if check_goal(state, condition):
                _compute_new_state_from_lifted_effects(state, lifted_effect.effect.literals, assignments, new_literals)
        else:
...
def _apply_effects(state, lifted_effects, assignments, get_all_transitions=False,
    ...
    for lifted_effect in lifted_effects:
        if isinstance(lifted_effect, ProbabilisticEffect):
            ...
            condition = ground_literal(lifted_effect.condition, assignments)
            if check_goal(state, condition):
                determinized_lifted_effects.append(lifted_effect.effect)
        else:
            determinized_lifted_effects.append(lifted_effect)

    states = []
    if not get_all_transitions:
        new_literals = _compute_new_state_from_lifted_effects(state, determinized_lifted_effects, assignments, new_literals)
    ...
    for prob_efs_combination in probabilistic_effects_combinations:
        ...
        new_prob_literals = _compute_new_state_from_lifted_effects(state, new_determinized_lifted_effects, assignments, new_prob_literals)
...
class PDDLEnv(gym.Env):
    ...
    def determinized(self):
        """
        Return the all-outcomes determinization of this environment
        """
        # assert (
        #     self._problem_index_fixed
        # ), "env problem index must be fixed using env.fix_problem_index(idx)"
        det = copy.deepcopy(self)
        for operator in self.domain.operators.keys():
            prob_effs = []
            det.domain.operators[operator].effects.literals = []
            for eff in self.domain.operators[operator].effects.literals:
                if isinstance(eff, ProbabilisticEffect):
                    prob_effs.append(eff.literals)
                else:
                    det.domain.operators[operator].effects.literals.append(eff)
            if prob_effs:
                outcomes = list(product(*prob_effs))
                for i in range(len(outcomes)):
                    det.domain.operators[operator + f"-{i}"] = copy.deepcopy(
                        det.domain.operators[operator]
                    )
                    det.domain.operators[operator + f"-{i}"].effects.literals.extend(
                        filter(lambda x: x != NoChange(), outcomes[i])
                    )
                    det.domain.operators[operator + f"-{i}"].name = operator + f"-{i}"
                del det.domain.operators[operator]
        det.domain.is_probabilistic = False
        return det
